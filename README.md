...
| 事前登録受付中 | ゲームニュース**記事本文の前半**の解析（受付中か終了か、事前登録特典、登録者数、App Store / Google Play の ID）＋ App Store 検索で予約注文中（`releaseDate` が未来）のタイトル | 公式サイトとストアへのリンク、特典・登録者数を併記 |

### 配信日の扱い

事前登録の配信日は誤りが出やすいため、次の優先順で決め、**どこの情報かをカードに明示**します。

1. **掲載元の記事見出し**に書かれた日付（`9月24日リリース`）。記事本文からは取りません。本文の後半には関連記事や別タイトルの日付が混ざり、無関係な日付を拾ってしまうためです
2. **App Store の予約ページ**の日付（`App Store 表記 2027年2月14日` と出典付きで表示）。12/31・1/1 や 1 年以上先の日付は「未定」を埋めるための仮日付とみなして使いません
3. どちらも無ければ **配信日未定**

見出しが「特別番組が9月17日20時より配信決定」のような場合は、作品自体の配信ではないため日付として採用しません（`番組` `放送` `生配信` `発表会` `ライブ` などを含む見出しは除外）。配信日が過ぎたタイトルは事前登録から自動的に外れます。
| 更新履歴 | 収集のたびに前回の状態と比較 | 新規・発売日決定・延期・前倒し・事前登録開始を直近 30 日分記録 |
