...
まず**同じ作品を 1 件にまとめます**。タイトル（空白・記号・ひらがな/カタカナの違いは無視）か App Store の ID が同じものは同じ作品です。以前は記事ごとに別々に載っていたため、同じ作品が 2〜3 件並んだり、1 件だけ「配信済み」で外れて残りが載り続けたりしていました（デイヴ・ザ・ダイバー、Lumi Master）。判定はまとめた単位で行い、どれか 1 本でも配信済みを示していれば作品ごと外します。

1. **App Store で予約注文中と確かめられたものは残す**（ストアの状態をいちばん信用します）
2. App Store で配信済みになっている
3. 公式サイトの説明文（og:description）に「配信中」「好評配信中」「今すぐダウンロード」「now available」などがある
4. 記事の見出しが「配信開始」「本日リリース」など配信を知らせるものになっている（「配信開始日が決定」「10月7日に配信開始決定」のような予告や、未来の日付つきのものは除く）
5. App Store で確認できず記事だけが根拠のものは、いちばん新しい記事の公開から `preregMaxAgeDays`（既定 45 日）を過ぎたら外す
6. 見出しから読み取った配信日が過去になっている

**App Store の状態は毎回取り直します。** 検索結果は 72 時間キャッシュしますが、キャッシュ時点の「予約中」をそのまま使うと、その間に配信が始まっても予約中のままになります（アニモが 9/23 の配信後も残った原因）。ID が分かっている作品は収集のたびに `lookup` API（50 件で 1 回の問い合わせ）で最新の配信日を確かめ、キャッシュから返す結果も配信日を過ぎていれば配信済みとして扱います。
...
- **自動収集（30 分おき）**
  ...
  - ニュース系フィードは直近 100 件程度しか持たないため、30 分おきに収集します（1 回あたり 10 秒前後。起動のしかたは `collect.yml` の冒頭を参照）
...
8. **リンク切れの非表示**: OGP 取得でリンク先の生存も確認し、404 / 410 / ソフト 404（タイトルが「ページが見つかりません」等）は公開 JSON から外します。ドメイン消滅・接続拒否は 2 回続いたら外します。403 / 429（ボット拒否）・タイムアウト・証明書チェーン不備はブラウザでは開けることが多いので残します。掲載済みの項目も `meta.recheckHours`（既定 24 時間）ごとに 1 回あたり `meta.recheckPerRun`（既定 40 件）ずつ見直し、復活すれば再表示します。1 で寄せたトップが 404 のときは、記事に載っていた元の URL（`linkUrl`）に切り替えます（`rootFallback`）。
9. **リンクの正規化**: Google 検索結果（`google.com/url?q=`）や Facebook のクッション URL は中身を取り出し、t.co などの短縮 URL は展開します。App Store は `apps.apple.com/{国}/app/id{ID}`（国指定なしは `jp`）、Google Play は `?id=` だけに整えます（アフィリエイトのパラメータが壊れていると 404 になるため）。

除外された項目は `data/last-run.json` の `offTopicItems` に、非表示にしたリンク切れは `deadLinkItems` に理由付きで記録されるので、行き過ぎた除外がないか確認できます。`siteFilter.enabled` を `false` にすると全て無効になります。
...
3. `.github/workflows/collect.yml` が 30 分おきに収集して `docs/radar/data/sites.json` をコミットします（Actions の書き込み権限が必要: Settings → Actions → General → Workflow permissions → Read and write）
